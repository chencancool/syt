...
class BizResponseError(Exception):
    ...
    def __str__(self):
        return "result_code: {}; error_code: {}; error_message: {}".format(self.ret_code, self.err_code, self.err_msg)


# from .exception import ServerError, ClientError, BizResponseError
# from .utils import md5_str, gen_rand_str
# from shouqianba import log
...
class ShouqianbaClient(BaseClient):
    ...
    def __init__(self, base_url=None, vendor_sn=None, vendor_key=None, terminal_sn=None, terminal_key=None, **kwargs):
        ...
        self.device_id = kwargs.pop('device_id', "50a87771-ca8a-4952-a493-9504c39ab491356")  # 设备唯一身份ID
        ...
        self.app_id = '2018051400000696'

        self.vendor_sn = vendor_sn or self.data['configs'].get('vendor_sn')
        self.vendor_key = vendor_key or self.data['configs'].get('vendor_key')
        self.terminal_key = terminal_key or self.data['configs'].get('terminal_key')
        self.terminal_sn = terminal_sn or self.data['configs'].get('terminal_sn')

        super(ShouqianbaClient, self).__init__(base_url or self.data['configs'].get('base_url'), **kwargs)
        self.json_handlers.append(result_code_checker)
        self.interceptor = lambda r, j: j

    def read_config(self, path='E:\\chencan\\work\\demo\\config.json'):
    ...
    def _make_signed_request_params(self, is_terminal_activation=False, body=None, **kwargs):
        ...
        sign = md5_str(body.encode('utf-8') + key.encode('utf-8'))
    ...
    def activate(self, code, override_terminal=False):
        ...
        if override_terminal and ret['result_code'] == "200" and ret['biz_response'].get("terminal_sn") and ret['biz_response'].get("terminal_key"):
            self.terminal_sn = ret['biz_response']['terminal_sn']
            self.terminal_key = ret['biz_response']['terminal_key']
            self.data['configs']['terminal_sn'] = self.terminal_sn
            self.data['configs']['terminal_key'] = self.terminal_key
            with open('E:\\chencan\\work\\demo\\config.json', 'w') as f:
    ...
    def checkin(self):
        ...
        if ret['result_code'] == "200" and ret['biz_response'].get("terminal_sn") and ret['biz_response'].get("terminal_key"):
            self.terminal_sn = ret['biz_response']['terminal_sn']
            self.terminal_key = ret['biz_response']['terminal_key']
            self.data['configs']['terminal_key'] = self.terminal_key
            with open('E:\\chencan\\work\\demo\\config.json', 'w') as f:
...
client = ShouqianbaClient(base_url=None, vendor_sn=None, vendor_key=None, terminal_sn=None, terminal_key=None)
